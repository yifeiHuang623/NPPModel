...
def train_model(model: flashback, train_dataloader: DataLoader, valid_dataloader: DataLoader, metric_keys, args):
...
def train(dataloader: dict[DataLoader], dataset_name, **kv):
    ...
    with open('./model/flashback/flashback.yaml', 'r', encoding='utf-8') as f:
...
def inference(dataloader: DataLoader, **kv):
    ...
    if kv.get('model_para', False):   
        ...
        log.info('=============args==============')
        log.info(args)
        model = flashback(args).to(args.device)
