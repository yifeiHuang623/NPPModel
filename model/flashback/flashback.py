...
from math import pi
from .flashback_utils import generate_tensor_of_distribution
...
class flashback(nn.Module):
    ...
    def forward(self, batch_data):        
        ...
        out_w = torch.zeros(seq_len, batch_size, self.hidden_size, device=x.device)
        for i in range(seq_len):
            sum_w = torch.zeros(batch_size, 1, device=x.device)
            for j in range(i+1):
                dist_t = torch.clamp(t[i] - t[j], min=0)
                dist_s = torch.norm(s[i] - s[j], dim=-1)
                a_j = self.f_t(dist_t, batch_size)
                b_j = self.f_s(dist_s, batch_size)
                a_j = a_j.unsqueeze(1)
                b_j = b_j.unsqueeze(1)
        ...
        out_pu = torch.zeros(seq_len, batch_size, 2*self.hidden_size, device=x.device)
        for i in range(seq_len):
