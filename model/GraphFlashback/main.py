...
from .GraphFlashback import GraphFlashback
from utils.EarlyStopping import EarlyStopping
from tqdm import tqdm
from utils.register import EVAL_REGISTRY
log = get_logger(__name__)

pre_views = ['GraphFlashback_preview']
post_views = []  
...
def train_model(model: GraphFlashback, train_dataloader: DataLoader, valid_dataloader: DataLoader, metric_keys, args):
...
def train(dataloader: dict[DataLoader], dataset_name, **kv):
    ...
    with open('./model/GraphFlashback/GraphFlashback.yaml', 'r', encoding='utf-8') as f:
...
def inference(dataloader: DataLoader, **kv):
    ...
    if kv.get('model_para', False):   
        ...
        log.info('=============args==============')
        log.info(args)
        model = GraphFlashback(args).to(args.device)
