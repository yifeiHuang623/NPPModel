...
def train_model(model: replay, train_dataloader: DataLoader, valid_dataloader: DataLoader, metric_keys, args):
...
def train(dataloader: dict[DataLoader], dataset_name, **kv):
    ...
    with open('./model/replay/replay.yaml', 'r', encoding='utf-8') as f:
...
def inference(dataloader: DataLoader, **kv):
    ...
    if kv.get('model_para', False):   
        ...
        log.info('=============args==============')
        log.info(args)
        model = replay(args).to(args.device)
