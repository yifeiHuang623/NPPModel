...
def pre_process_func() -> pd.DataFrame:
    ...
    df = df[["UTCTimeOffsetEpoch", "POI_id", "user_id", "latitude", "longitude"]]

    df["POI_id"] = pd.factorize(df["POI_id"])[0] + 1
    df["user_id"] = pd.factorize(df["user_id"])[0] + 1
    df["POI_catid"] = 0

    df["timestamps"] = pd.to_datetime(df["UTCTimeOffsetEpoch"], unit="s", errors="coerce").astype("int64")
...
@register_dataloader(name="foursquare_time_interval")
class foursquare_TimeInterval_DataLoader(BaseDataLoader):
    def __init__(self, model_args=None, pre_views=None, post_views=None) -> None:
        super().__init__(
            MyDataset=MyDatasetTimeInterval,
            dataset_name="foursquare_time_interval",
            logger=logger,
            args=args,
...
@register_dataloader(name="foursquare_fixed_length")
class foursquare_FixedLength_DataLoader(BaseDataLoader):
    def __init__(self, model_args=None, pre_views=None, post_views=None) -> None:
        super().__init__(
            MyDataset=MyDatasetFixedLength,
            dataset_name="foursquare_fixed_length",
            logger=logger,
            args=args,
            model_args=model_args or {},
            pre_views=pre_views,
...
@register_dataloader(name="foursquare_recent_k")
class foursquare_Recent_DataLoader(BaseDataLoader):
    def __init__(self, model_args=None, pre_views=None, post_views=None) -> None:
        super().__init__(
            MyDataset=MyDatasetRecentK,
            dataset_name="foursquare_recent_k",
            logger=logger,
            args=args,
            model_args=model_args or {},
            pre_views=pre_views,
...
if __name__ == "__main__":
    d = foursquare_TimeInterval_DataLoader(model_args={"batch_size": 32})
