...
def pre_process_func() -> pd.DataFrame:
    ...
    df = df[["UTCTimeOffsetEpoch", "POI_id", "user_id", "latitude", "longitude", "POI_catid"]]
...
@register_dataloader(name="TKY_time_interval")
class TKY_TimeInterval_DataLoader(BaseDataLoader):
    def __init__(self, model_args=None, pre_views=None, post_views=None) -> None:
        super().__init__(
            MyDataset=MyDatasetTimeInterval,
            dataset_name="TKY_time_interval",
            logger=logger,
            args=args,
...
@register_dataloader(name="TKY_fixed_length")
class TKY_FixedLength_DataLoader(BaseDataLoader):
    def __init__(self, model_args=None, pre_views=None, post_views=None) -> None:
        super().__init__(
            MyDataset=MyDatasetFixedLength,
            dataset_name="TKY_fixed_length",
            logger=logger,
            args=args,
            model_args=model_args or {},
            pre_views=pre_views,
...
@register_dataloader(name="TKY_recent_k")
class TKY_Recent_DataLoader(BaseDataLoader):
    def __init__(self, model_args=None, pre_views=None, post_views=None) -> None:
        super().__init__(
            MyDataset=MyDatasetRecentK,
            dataset_name="TKY_recent_k",
            logger=logger,
            args=args,
            model_args=model_args or {},
            pre_views=pre_views,
...
if __name__ == "__main__":
    d = TKY_TimeInterval_DataLoader(model_args={"batch_size": 32})
